...
<body>
 <?php
	session_start();
	if((empty($_SESSION['username'])) or ($_SESSION['area']!='bodega')){
		header('Location: ../login.php');
	}
	$conexion = new mysqli("localhost", "root", "", "gpi");
	$usuario = $_SESSION['username'];
	$area = $_SESSION['area'];
	if($area == 'obras'){$obra = $_SESSION['obra'];}
...
<section class="engine"><a href="https://mobiri.se/k">how to develop your own website</a></section><section class="mbr-section mbr-section__container article mbr-after-navbar" id="header3-v" data-rv-view="87" style="background-color: rgb(239, 239, 239); padding-top: 20px; padding-bottom: 0px;">
    <div class="container">
        <div class="row">
            <div class="col-xs-12">
                <h3 class="mbr-section-title display-2"><br><br>Solicitudes Pendientes</h3>
                <small class="mbr-section-subtitle"><p>Todas las solicitudes de todas las obras activas que estan pendientes para su revisión&nbsp;&nbsp;&nbsp;&nbsp;</p></small>
            </div>
        </div>
    </div>
</section>
...
<section class="mbr-section form2" id="form2-12" data-rv-view="116" style="background-color: rgb(239, 239, 239); padding-top: 20px; padding-bottom: 120px;">
    ...
    <div class="mbr-section mbr-section__container mbr-section__container--middle">
        <div class="container">
            <div class="row">
					...
					<table ><form class="mbr-form" method='post' action='reservar_solicitudes.php' >
	...
	<div class="mbr-section mbr-section-nopadding">
        <div class="container">
        	<div class="row">
				<?php $sql = "SELECT * FROM solicitud WHERE estado = 'pendiente'";
					  $result = $conexion->query($sql);?>
				<h3 align="center"	>Reservar materiales de Solicitud</h3><br>
				<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Seleccione una solicitud para verificar si los materiales solicitados estan disponibles en el inventario y reservarlos, o es necesario adquirir más.</p>
				<table ><form  class="mbr-form" method='post' action='reservar_solicitudes.php' class="form">
					<tr><td><label class="boton2"><br>Seleccione una Solicitud:</label></td><td><select name='solicitud_r' class="boton3">
					<?php while ($row = mysqli_fetch_assoc($result)){
						$numero = $row['numero'];
						echo "<option value='$numero'>$numero</option>";
					}?>
					</select></td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
					<td><span class="boton"><button type="submit" class="btn btn-primary">RESERVAR</button></span></td></tr>
				</form></table>
				<?php if(!empty($_POST['solicitud_r'])){
					$solicitud = $_POST['solicitud_r'];
					$sql = "SELECT * FROM productos_solicitados WHERE nro_solicitud = '$solicitud' AND estado = 'solicitado'";
					$result = $conexion->query( $sql );
					while ($row = mysqli_fetch_assoc($result)){	//Para cada producto solicitado se comprueba si hay stock
						$cantidad_solicitada = $row['cantidad'];
						$id_solicitado = $row['id'];
						$id_producto = $row['id_producto'];
						...
						if( $cantidad_solicitada > $cantidad_existente ){  //Hay materiales pero no suficientes
							$sql_update = "UPDATE productos_solicitados SET estado= 'enviado a adquisiciones' WHERE id='$id_solicitado'";
							$result_update = $conexion->query( $sql_update );
						}
						else{//Hay materiales suficientes
							$sql_update = "UPDATE productos_solicitados SET estado= 'reservado en bodega' WHERE id='$id_solicitado'";
							$result_update = $conexion->query( $sql_update );
							$diferencia = $cantidad_existente - $cantidad_solicitada;
							$sql_update = "UPDATE productos SET cantidad = '$diferencia' WHERE id = '$id_producto'";
							$result_update = $conexion->query( $sql_update );
						}
					}
					$sql_update = "UPDATE solicitud SET estado= 'reservada' WHERE numero='$solicitud'";
					$result_update = $conexion->query( $sql_update );
					header('Location: ../bodega/reservar_solicitudes.php');
				}
				?>
			</div>
		</div>
